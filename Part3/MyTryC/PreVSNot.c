// sbox_timing_aesstyle.c
//
// Compares timing of (AES first-round style):
//   1) Vulnerable: AddRoundKey -> SubBytes using real AES S-box table + leaky dummy
//   2) "CT toy":   AddRoundKey -> SubBytes using algebraic S-box + constant dummy
//
// Output CSV:
//   implementation,byte_value,time_ns,sample_id
//
// Compile (Linux/WSL):
//   gcc -O2 -march=native -Wall sbox_timing_aesstyle.c -o sbox_timing_aesstyle
//
// Compile (Windows MinGW):
//   gcc -O2 -march=native -Wall sbox_timing_aesstyle.c -o sbox_timing_aesstyle.exe
//
// Run:
//   ./sbox_timing_aesstyle
//
// Output:
//   sbox_precomputed_vs_algebraic_P3_v2.csv

#define _GNU_SOURCE

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#ifdef _WIN32
#include <windows.h>
#elif defined(__linux__)
#include <sched.h>
#include <unistd.h>
#endif

// ---------------- CONFIG ----------------

#define BLOCK_SIZE          16
#define NUM_PLAIN_VALUES    256

#define SAMPLES_PER_BYTE    50
#define LOOPS_PER_MEASURE   500

// ---------------- CPU PINNING (optional) ----------------

static void pin_to_cpu0(void) {
#ifdef _WIN32
    HANDLE hThread = GetCurrentThread();
    DWORD_PTR mask = 1; // CPU0
    DWORD_PTR result = SetThreadAffinityMask(hThread, mask);
    if (result == 0) {
        fprintf(stderr, "SetThreadAffinityMask failed\n");
    }
#elif defined(__linux__)
    cpu_set_t set;
    CPU_ZERO(&set);
    CPU_SET(0, &set);
    if (sched_setaffinity(0, sizeof(set), &set) != 0) {
        perror("sched_setaffinity");
    }
#else
    (void)0;
#endif
}

// ---------------- HIGH-RES TIMER IN NANOSECONDS ----------------

static double get_time_ns(void) {
#ifdef _WIN32
    static LARGE_INTEGER freq;
    static int freq_init = 0;
    LARGE_INTEGER counter;

    if (!freq_init) {
        if (!QueryPerformanceFrequency(&freq)) {
            return (double)GetTickCount() * 1e6; // coarse fallback
        }
        freq_init = 1;
    }

    QueryPerformanceCounter(&counter);
    return (double)counter.QuadPart * 1e9 / (double)freq.QuadPart;

#elif defined(__linux__)
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC_RAW, &ts);
    return (double)ts.tv_sec * 1e9 + (double)ts.tv_nsec;
#else
    return (double)clock() * (1e9 / (double)CLOCKS_PER_SEC);
#endif
}

// ---------------- REAL AES S-BOX (256 entries) ----------------

static const uint8_t AES_SBOX[256] = {
  
};

// ---------------- Algebraic S-box (toy CT) ----------------

static inline uint8_t rotl8(uint8_t x, unsigned n) {
    return (uint8_t)((x << n) | (x >> (8 - n)));
}

// Same as your Python _algebraic_sbox (not real AES S-box)
static inline uint8_t algebraic_sbox(uint8_t x) {
    uint8_t y = (uint8_t)(x * 17u);
    y ^= rotl8(x, 1);
    y ^= rotl8(x, 2);
    y ^= 0x5Au;
    return y;
}

// ---------------- Dummy work (match your Python loops) ----------------

static void leaky_dummy(uint8_t byte_val) {
    volatile int dummy = 0;

    if (byte_val > 200) {
        for (int x = 0; x < 20; x++) dummy += x;
    } else if (byte_val == 0) {
        for (int x = 0; x < 30; x++) dummy += x;
    } else {
        for (int x = 0; x < 2; x++) dummy += x;
    }
}

static void constant_dummy(uint8_t byte_val) {
    // matches your Python CT dummy loop count (=4)
    volatile int dummy = 0;
    for (int x = 0; x < 4; x++) dummy += (x ^ byte_val);
}

// ---------------- AES-like steps: AddRoundKey + SubBytes ----------------

static void add_round_key(uint8_t state[BLOCK_SIZE], const uint8_t key[BLOCK_SIZE]) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        state[i] ^= key[i];
    }
}

static void sub_bytes_vulnerable(uint8_t state[BLOCK_SIZE]) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        uint8_t val = state[i];          // val = (pt ^ key) like Python
        leaky_dummy(val);                // leak depends on val
        state[i] = AES_SBOX[val];        // real AES S-box lookup
    }
}

static void sub_bytes_algebraic(uint8_t state[BLOCK_SIZE]) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        uint8_t val = state[i];          // val = (pt ^ key)
        constant_dummy(val);             // fixed work
        state[i] = algebraic_sbox(val);  // computed, fixed operations
    }
}

// Encrypt = AddRoundKey + SubBytes (exactly like your Python)
static void encrypt_precomputed(const uint8_t key[BLOCK_SIZE],
                                const uint8_t pt[BLOCK_SIZE],
                                uint8_t ct[BLOCK_SIZE])
{
    uint8_t state[BLOCK_SIZE];
    memcpy(state, pt, BLOCK_SIZE);

    add_round_key(state, key);
    sub_bytes_vulnerable(state);

    memcpy(ct, state, BLOCK_SIZE);
}

static void encrypt_algebraic(const uint8_t key[BLOCK_SIZE],
                              const uint8_t pt[BLOCK_SIZE],
                              uint8_t ct[BLOCK_SIZE])
{
    uint8_t state[BLOCK_SIZE];
    memcpy(state, pt, BLOCK_SIZE);

    add_round_key(state, key);
    sub_bytes_algebraic(state);

    memcpy(ct, state, BLOCK_SIZE);
}

// ---------------- Measurement harness ----------------

typedef void (*encrypt_func_t)(const uint8_t key[BLOCK_SIZE],
                               const uint8_t pt[BLOCK_SIZE],
                               uint8_t ct[BLOCK_SIZE]);

static double measure_ns(encrypt_func_t f,
                         const uint8_t key[BLOCK_SIZE],
                         const uint8_t pt[BLOCK_SIZE],
                         int loops)
{
    uint8_t ct[BLOCK_SIZE];

    // warm-up
    f(key, pt, ct);

    double start = get_time_ns();
    for (int i = 0; i < loops; i++) {
        f(key, pt, ct);
    }
    double end = get_time_ns();

    return (end - start) / (double)loops;
}

// ---------------- MAIN ----------------

int main(void) {
    // pin_to_cpu0(); // enable if you want lower noise

    uint8_t key[BLOCK_SIZE] = {
        0x3C, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00
    };

    uint8_t pt[BLOCK_SIZE];

    FILE *fp = fopen("sbox_precomputed_vs_algebraic_P3_v2.csv", "w");
    if (!fp) {
        perror("fopen");
        return 1;
    }

    fprintf(fp, "implementation,byte_value,time_ns,sample_id\n");

    printf("Collecting data for %d byte values, %d samples each...\n",
           NUM_PLAIN_VALUES, SAMPLES_PER_BYTE);

    for (int byte = 0; byte < NUM_PLAIN_VALUES; byte++) {
        if (byte % 16 == 0) {
            printf("  progress: %d/%d\n", byte, NUM_PLAIN_VALUES - 1);
        }

        for (int s = 0; s < SAMPLES_PER_BYTE; s++) {
            pt[0] = (uint8_t)byte;
            for (int i = 1; i < BLOCK_SIZE; i++) pt[i] = 0x01;

            double ns_pre = measure_ns(encrypt_precomputed, key, pt, LOOPS_PER_MEASURE);
            fprintf(fp, "Precomputed_SBox,%d,%.2f,%d\n", byte, ns_pre, s);

            double ns_alg = measure_ns(encrypt_algebraic, key, pt, LOOPS_PER_MEASURE);
            fprintf(fp, "Algebraic_SBox,%d,%.2f,%d\n", byte, ns_alg, s);
        }
    }

    fclose(fp);
    printf("Done. Results written to sbox_precomputed_vs_algebraic_P3_v2.csv\n");
    return 0;
}
